...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $action) {
    ...
    if ($action === 'crear_observacion' && $role === 'instructor') {
        ...
        $db->prepare("INSERT INTO auditoria (usuario, accion) VALUES (?, ?)")->execute([$_SESSION['user']['username'], $auditAction]);
        header('Location: ?page=observaciones'); exit;
    }
}
    ...
    <main class="flex-1 p-8 overflow-y-auto">
        <?php if($page == 'dashboard'): ?>
            <div class="grid grid-cols-4 gap-4 mb-8">
                <?php foreach(['aprendices','instructores','riesgos','usuarios'] as $t): ?><div class="bg-white p-4 shadow"><?=ucfirst($t)?>: <b><?=$db->query("SELECT count(*) FROM $t")->fetchColumn()?></b></div><?php endforeach; ?>
            </div>
            <div class="grid grid-cols-3 gap-4">
                <?php foreach($db->query("SELECT * FROM riesgos") as $r): 
                    ...
                    <div class="bg-white p-4 border-l-4 border-<?=$color?>-500 shadow">
                        <p class="font-bold"><?=$r['descripcion']?></p><p class="text-xs">Nivel: <?=$r['nivel']?></p>
                    </div>
                <?php endforeach; ?>
            </div>
        <?php endif; ?>
        ...
        <?php if($page == 'usuarios' && $_SESSION['user']['role'] == 'admin'): ?>
            ...
            <div class="bg-white p-4 shadow rounded">
                ...
                <table class="w-full text-left border-collapse">
                    <thead><tr><th class="p-2 border">ID</th><th class="p-2 border">Usuario</th><th class="p-2 border">Rol</th></tr></thead>
                    <tbody>
            ...
            <table class="w-full bg-white shadow">
                <?php foreach($db->query("SELECT * FROM aprendices") as $a): ?>
                    <tr>
                        ...
                        <?php if($_SESSION['user']['role'] == 'admin'): ?>
                            <td><a href="?page=aprendices&action=del&tabla=aprendices&id=<?=$a['id']?>" class="text-red-500">Eliminar</a></td>
                        <?php endif; ?>
                    </tr>
                <?php endforeach; ?>
            </table>
        ...
        <?php if($page == 'instructores' && $_SESSION['user']['role'] == 'admin'): ?>
            ...
            <div class="bg-white p-4 shadow rounded">
                ...
                <table class="w-full text-left border-collapse">
                    <thead><tr><th class="p-2 border">Nombre</th><th class="p-2 border">Especialidad</th><th class="p-2 border">Acciones</th></tr></thead>
                    <tbody>
                        <?php foreach($db->query("SELECT * FROM instructores") as $i): ?>
                            <tr>
                                ...
                                <td class="p-2 border"><a href="?page=instructores&action=del&tabla=instructores&id=<?=$i['id']?>" class="text-red-500">Eliminar</a></td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
        ...
        <?php if($page == 'auditoria' && $_SESSION['user']['role'] == 'admin'): ?>
            <div class="bg-white p-4 shadow rounded">
                ...
                <table class="w-full text-left border-collapse">
                    <thead><tr><th class="p-2 border">Fecha</th><th class="p-2 border">Usuario</th><th class="p-2 border">Acción</th></tr></thead>
                    <tbody>
        ...
        <?php if($page == 'riesgos'): ?>
            ...
            ?>
            <?php if($_SESSION['user']['role'] === 'admin'): ?>
                <form method="POST" action="?page=riesgos&action=<?= $editRisk ? 'editar_riesgo' : 'crear_riesgo' ?>" class="grid grid-cols-2 gap-2 bg-white p-4 shadow mb-4">
                    <input name="desc" placeholder="Descripción" class="col-span-2 border p-2" required value="<?= $editRisk ? htmlspecialchars($editRisk['descripcion']) : '' ?>">
                    <input type="number" name="prob" placeholder="Prob (1-3)" class="border p-2" min="1" max="3" required value="<?= $editRisk ? max(1, min(3, ceil($editRisk['nivel'] / 2))) : '' ?>">
                    <input type="number" name="imp" placeholder="Imp (1-3)" class="border p-2" min="1" max="3" required value="<?= $editRisk ? max(1, min(3, ceil($editRisk['nivel'] / 2))) : '' ?>">
                    <textarea name="just" placeholder="Justificación" class="col-span-2 border p-2" required><?= $editRisk ? htmlspecialchars($editRisk['justificacion']) : '' ?></textarea>
                    <?php if($editRisk): ?><input type="hidden" name="id" value="<?= $editRisk['id'] ?>"><?php endif; ?>
                    <button class="col-span-2 bg-red-600 text-white p-2"><?= $editRisk ? 'Actualizar riesgo' : 'Guardar riesgo' ?></button>
                </form>
            <?php endif; ?>
            <div class="bg-white p-4 shadow rounded">
                <h2 class="font-bold mb-3">Listado de riesgos</h2>
                <table class="w-full text-left border-collapse">
                    <thead><tr><th class="p-2 border">Descripción</th><th class="p-2 border">Nivel</th><th class="p-2 border">Justificación</th><?php if($_SESSION['user']['role'] === 'admin'): ?><th class="p-2 border">Acciones</th><?php endif; ?></tr></thead>
                    <tbody>
                        <?php foreach($db->query("SELECT * FROM riesgos") as $r): ?>
                            ...
                            <tr>
                                ...
                                <?php if($_SESSION['user']['role'] === 'admin'): ?>
                                    <td class="p-2 border flex gap-2">
                                        ...
                                        <a href="?page=riesgos&action=del&tabla=riesgos&id=<?=$r['id']?>" class="text-red-500">Eliminar</a>
                                    </td>
                                <?php endif; ?>
                            </tr>
                        <?php endforeach; ?>
        ...
        <?php if($page == 'observaciones'): ?>
            ...
            <div class="bg-white p-4 shadow rounded">
                ...
                <table class="w-full text-left border-collapse">
                    <thead><tr><th class="p-2 border">Aprendiz</th><th class="p-2 border">Observación</th><th class="p-2 border">Autor</th></tr></thead>
                    <tbody>
                        <?php
